...
namespace Keyboard
{
    ...
    public class Messaging
    {
        ...
        [Obsolete]
        public static void ClickOnPoint(IntPtr wndHandle, Point clientPoint, int delay = 100, bool restore = true)
        {
        ...
        public static void ClickOnPoint(IntPtr wndHandle, Point pos, int button = 0, int delay = 40, bool restore = true)
        {
            Point current;
            ...
            {
                throw new ArgumentException("Invalid argument value for button.");
            }

            GetCursorPos(out current);
            ClientToScreen(wndHandle, ref pos);

            // Move event.
            mouse_event(
                0x8000 | 0x0001,
                (int)Smoothstep((float)pos.X, 0, (float)resolution.Width, 0, 65535),
                (int)Smoothstep((float)pos.Y, 0, (float)resolution.Height, 0, 65535), 0, UIntPtr.Zero);
            ...
            if (restore)
                mouse_event(
                    0x8000 | 0x0001,
                    (int)Smoothstep(current.X, 0, resolution.Width, 0, 65535),
                    (int)Smoothstep(current.Y, 0, resolution.Height, 0, 65535), 0, UIntPtr.Zero);
        }

        public static float Smoothstep(float x, float a1, float a2, float c1, float c2)
        {
        ...
        public static void SendChatTextSend(IntPtr hWnd, string msg, bool sendReturn = false)
        {
